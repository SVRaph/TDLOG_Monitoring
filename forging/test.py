...
def ping_arp(nretry=1,s="172.24",lk=[200,201,202,203]):
    ...
    for k in xrange(101):
        k=k+99
        ans1,unans=srp(Ether(dst="ff:ff:ff:ff:ff:ff")/ARP(pdst=("%s.%d.0-63"    % (s,k)) ),retry=nretry, timeout=1)
        ans2,unans=srp(Ether(dst="ff:ff:ff:ff:ff:ff")/ARP(pdst=("%s.%d.64-127"  % (s,k)) ),retry=nretry, timeout=1)
        ans3,unans=srp(Ether(dst="ff:ff:ff:ff:ff:ff")/ARP(pdst=("%s.%d.128-191" % (s,k)) ),retry=nretry, timeout=1)
        ans4,unans=srp(Ether(dst="ff:ff:ff:ff:ff:ff")/ARP(pdst=("%s.%d.192-255" % (s,k)) ),retry=nretry, timeout=1)
        l=l+ans1+ans2+ans3+ans4
...
#essai pour envoyer un paquet à une adresse ip hors domaine
def test_arping2(macrezo='00:11:2f:40:d9:9b',macswitch='00:01:e7:c6:f1:80',mactouareg='00:18:f3:c6:6d:e4'):
    conf.checkIPaddr = False
    pkt=Ether(dst='00:08:D3:10:6A:48')/ARP(pdst="192.168.1.1",psrc='172.24.0.21',op="who-has")
